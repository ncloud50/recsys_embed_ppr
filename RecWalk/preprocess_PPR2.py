...
def mk_slim_df(user_item_train_df, user_item_test_df, user_list, item_list):
    ...
    user_item_test_slim_df.to_csv('./data2/user_item_test_slim.csv', index=False)
    user_item_train_slim_df.to_csv('./data2/user_item_train_slim.csv', index=False)
...
if __name__ == '__main__':
    ...
    relation_type = ['u_buy_i', 'i_belong_b']
    ...
    # 保存
    with open('./data2/user_list.txt', 'w') as f:
        for user in user_list:
            f.write(user + '\n')
    with open('./data2/item_list.txt', 'w') as f:
        for item in item_list:
            f.write(item + '\n')
    with open('./data2/brand_list.txt', 'w') as f:
    ...
    # 保存
    with open('./data2/entity_list.txt', 'w') as f:
    ...
    #保存
    triplet_df.to_csv('./data2/triplet.csv', index=False)
    ...
    with open('./data2/user_items_test_dict.pickle', 'wb') as f:
