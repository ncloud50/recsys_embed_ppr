...
import models
from models import DistMulti, TransE, SparseTransE, Complex, RegComplex
...
def objective(trial):
    ...
    alpha = trial.suggest_loguniform('alpha', 1e-6, 1e-2) #SparseTransE, RegComplexの時だけ
    ...
    # データ読み込み
        ...
        model = RegComplex(int(embedding_dim), relation_size, entity_size, alpha).to(device)
...
if __name__ == '__main__':
    ...
    df.to_csv('./result_luxury_2cross/hyparams_result_regComplex.csv')
    with open('./result_luxury_2cross/best_param_regComplex.pickle', 'wb') as f:
